...
- **Rattachement (implémenté — session du 07/08/2026, assoupli le 2026-09-25) : `project` et/ou `team`, jamais aucun des deux.** Les deux champs sont nullable en base (`null=True, blank=True`) — la règle est service-layer (`create_incident`, `apps/incidents/services.py`), pas une contrainte DB, cohérent avec le pattern déjà en place pour `create_project`. **Assoupli le 2026-09-25** : la règle initiale exigeait un XOR strict ("jamais les deux"), ce qui rejetait à tort un appel qui connaît légitimement les deux à la fois (retour direct en testant le flow ticketing/QWEASE réel : "on peut connaître le groupe ET le projet"). Recevoir les deux n'est plus une erreur — `team` est alors silencieusement ignoré (mis à `None` avant création), `project` prime, cohérent avec le fait qu'un projet collaboratif connaît déjà son groupe via `Project.team`. Aucune vérification de cohérence entre le `team` envoyé et `project.team` (choix délibéré, pas un oubli — voir le service). Motivation d'origine inchangée : l'outil de ticketing externe peut créer un incident sans connaître de projet Awtodo précis, en renseignant seulement le groupe (obligatoire côté outil externe, pas dans le modèle Awtodo) — voir "Boîte de réception des incidents non-affectés" ci-dessous.
- Titre, description
- Priorité
...
- `GET /api/v1/incidents/inbox/` — incidents `project=NULL` dont le `team` fait partie des groupes accessibles à l'acteur (`accessible_inbox_teams`, `apps/incidents/services.py` : admin plateforme voit tout, sinon uniquement les groupes où l'appartenance `TeamMembership` est active — même logique que `accessible_projects`, côté groupe). Réutilise `IncidentFilterSet` pour affiner (ex. `?status=signale`), mais **exclut toujours les statuts terminaux (`annulé`/`archivé`) en amont, indépendamment de ce filtre** (`status__in=Incident.ACTIVE_STATUSES` posé directement sur le queryset — session du 2026-09-25, retour direct : "je peux pas juste mettre une règle sur la boîte de réception qui filtre les statuts annulé ?"). Contrairement à la liste principale (`list()`), où consulter l'historique via un statut explicite reste légitime, un incident non-affecté n'a de sens à figurer dans la boîte de réception que tant qu'il reste à trier — même `?status=annule` explicite ne le fait plus réapparaître. Corrige un bug remonté juste avant (un incident annulé restait affiché après l'action, retiré côté frontend mais pas exclu côté backend par défaut) sans dépendre de la synchronisation du filtre de statut envoyé par le front.
...
**`POST /api/v1/incidents/{id}/reassign-team/` (implémenté — session du 2026-09-25)** : demande directe en testant le flow Power Automate/QWEASE (voir "Synchronisation Outlook" plus haut pour un autre exemple de ce genre de test réel) — "il faut pouvoir changer d'équipe à un incident, au cas où il arriverait dans la mauvaise équipe" (mauvais mappage côté intégration ticketing, ex. `CustomField` mal renseigné). Body `{"team": "<uuid>"}`. Reste dans une boîte de réception (`project` inchangé, `None`), change seulement `team` — distinct de `assign-project`, qui en sort définitivement. Nouvelle garde `_ensure_can_reassign_team` : `_require_member` sur le groupe **source** (même niveau d'autorité que le reste du cycle de vie d'un incident non-affecté — n'importe quel membre, pas réservé à un administrateur), plus une vérification d'appartenance au groupe de **destination** dans `reassign_incident_team` elle-même (même patron à deux niveaux que `assign_incident_to_project`). 400 si l'incident a déjà un projet (n'est plus dans une boîte de réception). Nouveau flag `permissions.can_reassign_team`. Frontend : bloc symétrique au rattachement de projet dans `IncidentAccordion.tsx` (sélecteur de groupe + bouton "Déplacer vers ce groupe"), nouvelle liste `teams` chargée par `IncidentsPage` (`getTeams()`, seulement hors projet scopé).
...
| `POST /api/v1/tasks/{id}/update-estimated-hours/` | membre du projet | modifie/efface le temps estimé |
| `POST /api/v1/tasks/{id}/update-deadline/` (session du 2026-09-22) | **chef de projet uniquement** | modifie/supprime l'échéance |
| `POST /api/v1/tasks/{id}/cancel/` (session du 2026-09-24) | **chef de projet uniquement** | `disponible`/`assignée`/`en_cours` → `annulée`, motif obligatoire |
...
**`POST /api/v1/tasks/{id}/update-deadline/` (implémenté — session du 2026-09-22) :** demande directe, "pouvoir changer/supprimer une date d'échéance sur une tâche, droit propre au rôle chef de projet" — à la différence du titre/description/temps estimé (`_ensure_can_rename`, tout membre du projet), la nouvelle fonction de garde `_ensure_can_edit_deadline` appelle `_require_manager` (même garde que `validate`/`reject`/`assign`) : réservé au chef de projet **de ce projet précis**. `deadline` accepte une chaîne `"YYYY-MM-DD"`, `null`/absent/chaîne vide pour supprimer l'échéance (parsing et validation faits dans le service, pas de serializer dédié — même convention que `update-estimated-hours`), ou une date invalide → 400. Nouveau flag `permissions.can_edit_deadline`. **Frontend** : `TaskAccordion.tsx` (partagé par `TasksListPage`/`TaskCardDialog`) et `TaskDrawer.tsx` (Kanban/Roadmap) gagnent tous deux une section "Échéance" — `DatePickerField` + bouton "Effacer" (le composant n'a pas de clear intégré) si `can_edit_deadline`, sinon texte en lecture seule. `apps/tasks/tests/test_deadline.py` (8 tests, service + API).

**`POST /api/v1/tasks/{id}/cancel/` (implémenté — session du 2026-09-24) :** demande directe, "il faut intégrer la possibilité d'annuler une tâche ou un incident" — investigation préalable a montré qu'aucun des deux modèles n'avait de vraie voie d'abandon depuis un état actif : `rejetée` (`reject_task`) ne s'applique qu'à `en_attente_validation` (refus avant même de démarrer), et `archivée` est en réalité la clôture réussie (posée par `complete_task`, pas une fonction d'archivage distincte). Nouveau statut terminal `annulée` (hors `ACTIVE_STATUSES`, comme `rejetée`/`archivée`), nouveau champ `Task.cancellation_reason` (même patron que `rejection_reason`). Nouvelle garde `_ensure_can_cancel` : même autorité que le rejet (`_require_manager` — chef de projet du projet concerné), applicable uniquement depuis `disponible`/`assignée`/`en_cours` (`en_attente_validation` reste couverte par le rejet, pas de chevauchement des deux transitions). Motif obligatoire, comme pour `reject`. Nouveau flag `permissions.can_cancel`. **Frontend** : nouveau composant partagé `components/CancelDialog.tsx` (copie du patron `RejectDialog.tsx`, généralisé sur un simple titre puisque réutilisé par les tâches et les incidents) ; bouton "Annuler" ajouté partout où "Rejeter" existait déjà côté tâches (`TaskCard` du Kanban, `TaskDrawer`, `TaskAccordion`) et branché dans `useTaskTransitions` (hook partagé par `KanbanBoard`/`RoadmapView`/`TasksListPage`/`TaskCardDialog`). Une tâche annulée n'émet aucun signal (`task_completed`/`task_assigned`) — n'entre donc jamais dans la file "à documenter" de `apps.documentation`, contrairement à une clôture réussie.
...
| `POST /api/v1/incidents/{id}/cancel/` (session du 2026-09-24), body `{"cancellation_reason": "..."}` | n'importe quel membre du groupe | `signalé`/`en_cours` → `annulé`, motif obligatoire |
...
**`POST /api/v1/incidents/{id}/cancel/` (implémenté — session du 2026-09-24) :** même demande que côté tâches (voir "API — endpoints de transition de statut (tâches)" > `cancel`), même investigation préalable : `archive` (`_ensure_can_archive`) exige `status == "résolu"`, donc aucune voie n'existait pour abandonner un incident `signalé`/`en_cours` qui s'avère invalide, doublon ou sans suite à donner — le seul choix était de le laisser traîner. Nouveau statut terminal `annulé` (hors `ACTIVE_STATUSES`), nouveau champ `Incident.cancellation_reason`. Nouvelle garde `_ensure_can_cancel` : même niveau d'autorité que le reste du cycle de vie (`_require_member`, pas réservé à un administrateur du groupe comme `update-priority`) — cohérent avec la "différence volontaire avec les tâches" ci-dessus (n'importe quel membre du groupe fait avancer le statut). Applicable uniquement depuis `signalé`/`en_cours` — un incident déjà `résolu` se clôture via `archive`, pas via une annulation. Motif obligatoire. Nouveau flag `permissions.can_cancel`. **Frontend** : bouton "Annuler" ajouté à la ligne de `IncidentsPage` (liste principale et boîte de réception) et à `IncidentAccordion`, réutilise `components/CancelDialog.tsx` (voir ci-dessus, partagé avec les tâches).
...
- **Frontend (`SpecTab.tsx`) — accordéon plutôt qu'un sommaire à cocher (révisé — session du 2026-09-21).** Retour direct : "plutôt que cocher sur les cases à déplier, ce serait mieux d'avoir un accordéon". Déplier/replier une section **est** désormais l'action d'inclusion — ouvrir une section vide l'ajoute au cahier des charges (`is_active=true`), la replier l'en retire, remplaçant la case à cocher du sommaire deux colonnes précédent. Pour un chef de projet (`can_edit_spec`), les 12 sections sont listées dans un `Accordion` **contrôlé** (`value`=sections actives, `onValueChange` déclenche `PATCH .../spec-sections/{key}/` avec `is_active`) — chaque section ouverte contient son contenu (même pattern lecture/édition que le bloc-notes : `MarkdownView` en lecture, `<textarea>` + Enregistrer/Annuler en édition, bouton "Modifier" en haut de la section). Pour un non-éditeur, seules les sections déjà actives sont listées, dans un `Accordion` **non contrôlé** (déplier/replier reste possible pour lire, sans aucun appel réseau ni effet sur `is_active` — la case à cocher désactivée-mais-visible de l'ancienne version n'a plus d'équivalent, un lecteur ne peut simplement pas faire apparaître une section vide).
- **`components/Accordion.tsx`** (nouveau, générique) : idée reprise d'un snippet Base UI fourni, réécrite aux conventions du repo (pas de Tailwind, pas de `@/components/base-ui/*`, pas de Radix) — composition `Accordion`/`AccordionItem`/`AccordionTrigger`/`AccordionContent`, anim `motion` (hauteur), mode contrôlé ou non contrôlé (`value`/`onValueChange` optionnels). Réutilisé aussi par le panneau "À planifier" du planning (voir plus bas dans ce fichier).
...
- **Horodatage.** `DocPage`/`DocEntry` exposent désormais `created_at`/`updated_at` dans leurs dicts (`TimeStampedModel` les portait déjà, seul l'exposé manquait) — affiché dans `DocumentationTab.tsx` (date de dernière modification d'une page, date de création d'une fiche) et sur la page publique (date sous chaque fiche).
- **Versionning des fiches.** `DocEntry.version` (FK nullable vers `projects.ProjectVersion`, `on_delete=SET_NULL`) — "faire remonter des fiches pour le versionning : ajout de la V1, ajout de la V2". Renseigné automatiquement à la version de la tâche source quand une fiche vient de la file "à documenter" (`create_entry_from_pending`, `pending.task.version` — un incident n'a pas de version, `None` dans ce cas). Modifiable à la main sinon (`update_entry(version=...)`, sentinelle `_UNSET` : `None` explicite détache la fiche de toute version, absent = inchangé). Exposé côté public (`version_label`) et affiché en badge sur les fiches publiées.
- **Recherche.** Pas de nouvel endpoint — un filtre texte 100 % frontend (`DocumentationTab.tsx`, sur `title`/`content` ou `title`/`description`) suffit au volume de contenu par projet ; le bundle est déjà chargé intégralement en un seul appel.
- **Images dans les pages.** Pas de stockage de fichiers disponible (S3/boto3 explicitement bloqué tant que l'infra AWS n'est pas détaillée, voir CLAUDE.md > Stack technique) — tranché avec l'utilisateur : les images sont encodées en base64 et embarquées directement dans le Markdown (`![alt](data:image/...;base64,...)`), zéro dépendance/infra supplémentaire, plafonnées à ~1,5 Mo côté frontend pour limiter le gonflement de la page. Purement frontend, `DocPage.content` est déjà un `TextField` sans limite de taille pratique.
- **Notifications au chef de projet.** Nouveau verb `Notification.verb = "doc_entry_pending"` — envoyé à **chaque** chef de projet actif du projet (un projet collaboratif peut en avoir plusieurs) dès qu'une ligne est réellement créée dans la file "à documenter" (`apps.documentation.signals`, `apps.notifications.services.notify_doc_entry_pending` — réutilise les FK `task`/`incident` déjà existantes sur `Notification`, pas de nouveau champ). `apps.documentation` importe directement `apps.notifications` (plus bas dans la hiérarchie de dépendances, sens autorisé).
- **Fiche « Contributeurs au projet ».** Nouvelle valeur `DocEntry.kind = "contributeurs"` — une seule fiche par espace (`get_or_create` sur `(space, kind)`), régénérée à la demande (`generate_contributors_entry`, `POST .../contributors/generate/`) : c'est un instantané de l'équipe active au moment de l'appel (liste par rôle, chef de projet/membre/lecteur), pas un journal — aucune notion de "à documenter" pour ce kind.
- **Personnalisation de la page publique.** Nouveaux champs `DocSpace.accent_color`/`header_content`/`footer_content` (`update_space_appearance`, `PATCH .../appearance/`) — **s'applique uniquement à la page publique**, l'écran d'édition dans Awtodo garde l'habillage standard. Couleur appliquée en overridant le jeton CSS `--color-accent` sur le conteneur racine de `PublicDocsPage` (cascade pure, pas de nouvelle classe par couleur).
- **URL personnalisée du lien public.** Décision explicite avec l'utilisateur : le token reste la seule source de non-devinabilité — `DocSpace.custom_slug` n'est qu'un **préfixe cosmétique lisible** embarqué dans `public_token` lui-même (`_generate_token(slug)` → `f"{slug}-{secrets.token_urlsafe(24)}"`), jamais une clé de recherche séparée. `set_public_slug` (`PATCH .../public-link/slug/`) régénère `public_token` s'il existe déjà (même principe qu'une rotation — l'ancien lien cesse de fonctionner), sinon se contente de mémoriser le slug pour la prochaine activation.
...
| `GET/POST /planning/events/` | liste des séries dont l'utilisateur est owner ou participant actif / création (`title`, `start`, `end`, `all_day?`, `description?`, `location?`, `recurrence_rule?`, `participant_ids?` — voir "Participants dès la création" ci-dessous) |
...
| `POST /planning/events/{id}/occurrences/update/` · `.../occurrences/cancel/` | modifie/annule UNE occurrence (`occurrence_start` en body), sans toucher au reste de la série — session du 2026-09-23, voir "Synchronisation Outlook" > "Occurrence unique" |
...
Nouvel onglet « Communication » du hub projet (`frontend/src/features/communication/CommunicationTab.tsx`), réservé aux contributeurs. Trois blocs à l'origine : (1) connexion Office 365 — résumé + formulaire d'édition réservé à un admin d'organisation (`currentUser.is_platform_admin || organisation_role === "admin"`) ; (2) canaux du projet — liste + formulaire d'ajout (chef de projet) avec `Combobox` pour le type et `Checkbox` (switch) pour `notify_incident_created` ; (3) rédaction (contributeurs, sélection des destinataires via une liste de switches — pas un `Combobox`, la sélection est multiple) + historique des messages (`StatusBadge` neutre, icône par statut). Bandeau explicite rappelant que l'envoi réel n'est pas encore actif. 532 → 544 tests backend, tous verts.
**⚠️ Bloc (1) scindé en deux — session du 2026-09-22 (suite Outlook), voir plus bas** : le formulaire d'édition de `O365Connection` n'avait plus sa place tel quel sur un onglet **par projet** alors que la connexion est **par organisation** — remonté directement par l'utilisateur ("on fout pas ça dans Communication"). `tenant_id`/`client_id`/`client_secret`/`is_enabled` déplacés vers `Administration > Intégrations` (nouvel écran, admin d'organisation uniquement). **`sender_mailbox` reste dans l'onglet Communication du projet** (remonté juste après : "pas tout le monde n'utilise la même [boîte]") — édité ici, admin d'organisation toujours (même endpoint `PATCH /api/v1/communication/o365/`, même garde `is_organisation_admin`, aucun assouplissement de permission). **Limite backend assumée, non résolue** : le modèle `O365Connection` reste `OneToOneField(organisation)`, une seule valeur de `sender_mailbox` pour toute l'organisation malgré l'édition "par projet" côté écran — une vraie boîte distincte par projet nécessiterait sa propre autorisation d'envoi Graph (`Send As`/`Send on Behalf`), explicitement mis en attente par l'utilisateur, pas dans le périmètre de cette session (le chantier Communication au complet reste différé).

**Limite v1 levée (session du 2026-09-21)** : le panneau « À planifier » liste désormais aussi les **incidents**, pas seulement les tâches — voir « Planning — panneau "À planifier" : tâches et incidents » plus bas. `Incident.assigned_to` était en réalité déjà exposé par `IncidentSerializer`, seul le filtre de requête `assigned_to` manquait côté `IncidentFilterSet`.
...
### Planning — couleur du calendrier personnel

**Statut : implémenté (session du 2026-09-18).** `User.planning_color` (`CharField(7)`, blank, validé `^#[0-9A-Fa-f]{6}$` ou vide — vide = repli sur l'accent thémé, comportement inchangé pour qui n'a jamais rien réglé), exposé sur `UserSerializer` de base (pas seulement `MeSerializer` — pas une donnée privée, contrairement à `email_notifications_enabled` : un calendrier partagé pourrait un jour afficher la couleur choisie par son propriétaire, `CalendarUser` allégé d'`apps.planning` ne l'expose pas encore, seul l'usage "ma propre vue" est câblé). `PATCH /api/v1/accounts/me/planning-preferences/` (`{"planning_color": "#RRGGBB" | ""}`). Palette restreinte à 6 teintes sobres (`PLANNING_PALETTE`, `features/planning/types.ts`, partagée avec la rotation des calendriers partagés) plutôt qu'un `<input type="color">` natif — impossible à styler pour respecter la charte, risque de couleurs criardes hors palette.

### Planning — horaires de travail (modèle hebdomadaire, exceptions, délégation)

**Statut : implémenté, remplace une première version plus simple de la même session (un seul horaire pour toute la semaine, `User.work_hours_start/end`) — retours directs successifs : "on doit pouvoir modifier jour par jour", "on doit pouvoir le faire semaine par semaine", "on doit pouvoir donner le droit à quelqu'un de le faire lorsqu'on fait partage".** Les champs `User.work_hours_start/end` sont retirés (migration `accounts.0016`) ; le concept déménage entièrement dans `apps.planning` (`apps.planning` dépend déjà de `apps.accounts`, jamais l'inverse — cohérent avec la règle de dépendances entre apps).

**Modèles (`apps/planning/models.py`)** :
- `WorkingHoursDay` : une ligne par (`user`, `weekday` 0=lundi..6=dimanche) — `enabled`, `start`, `end` (`TimeField`, défaut `09:00`/`18:00`). Les 7 lignes d'un utilisateur sont créées à la demande (`get_or_create` paresseux, lundi-vendredi actifs par défaut) plutôt que via un signal `post_save` sur `User`, pour ne pas coupler `apps.accounts` à `apps.planning`.
- `WorkingHoursWeekOverride` (`user`, `week_start` = lundi de la semaine) + `WorkingHoursOverrideDay` (même forme que `WorkingHoursDay`, `FK` vers l'override) : exception ponctuelle pour une semaine calendaire précise, prime sur le modèle de base quand elle existe. Supprimée par une vraie suppression (pas un statut) quand l'utilisateur retire l'exception — ce ne sont pas des entités métier à historiser, même raisonnement que `User.planning_color` (champ overwritable sans cycle de vie).
- `CalendarShare.can_manage_work_hours` (booléen, défaut `False`) : droit additionnel sur un partage déjà existant — le bénéficiaire peut alors modifier les horaires du propriétaire, pas seulement voir son calendrier.

**Permissions (`apps/planning/services.py`)** : `_ensure_can_view_working_hours` (soi-même, ou n'importe quel partage actif) / `_ensure_can_manage_working_hours` (soi-même, ou un partage actif avec `can_manage_work_hours=True`). Les horaires ne sont donc jamais visibles/modifiables par un inconnu, contrairement à `planning_color`.

### API — `/api/v1/planning/`

| Méthode / chemin | Effet |
|---|---|
| `GET /working-hours/?user=<id>&week=<YYYY-MM-DD>` | `user` optionnel (défaut soi-même, sinon requiert un partage `can_manage_work_hours`) ; `week` optionnel (lundi visé) — si une exception existe pour cette semaine elle prime, sinon repli sur le modèle de base (`is_override` dans la réponse) |
| `PATCH /working-hours/` | `{"days": [{"weekday", "enabled", "start", "end"}, …], "user"?, "week_start"?}` — sans `week_start` : modifie le modèle de base ; avec : crée/met à jour l'exception de cette semaine |
| `DELETE /working-hours/?week=<date>&user=<id>` | retire l'exception de cette semaine (retour au modèle de base) |
| `POST /shares/` | gagne `can_manage_work_hours` (bool, défaut `False`) dans le body de création |
| `PATCH /shares/{id}/permissions/` | `{"can_manage_work_hours": bool}` — réservé au propriétaire du partage (`owner`), pas au bénéficiaire |

**Frontend** : `PlanningPreferencesDialog.tsx` (bouton "Mon planning" sur l'écran Planning) — un sélecteur de personne (`Combobox`, visible seulement si des partages délégués existent) pour éditer ses propres horaires ou ceux d'un tiers délégué ; un bascule "Toutes les semaines" / "Cette semaine seulement" (`scope`) ; 7 lignes jour × case à cocher (travaillé/non) + deux `<input type="time">`. `SharePanel.tsx` gagne une case "Peut aussi gérer mes horaires de travail" à la création d'un partage, et un toggle par ligne pour les partages déjà accordés. `WeekGrid.tsx` reçoit les 7 jours de la semaine affichée (refetchés à chaque changement de semaine, `PlanningPage`) et grise (`.week-grid__offhours`, visuel seulement — ne bloque ni la création ni le glisser-déposer) soit la colonne entière (jour non travaillé), soit les bandes avant/après la plage réglée, par jour de la grille.

**Tests** : `apps/planning/tests/test_working_hours.py` (modèle de base, exceptions, délégation, permissions — service + API), 15 tests.

### Planning — accès à une tâche/incident depuis un créneau

`BlockDialog.tsx` affiche un lien "Ouvrir la tâche"/"Ouvrir l'incident" quand le créneau cliqué (`ScheduledBlock`) est rattaché à l'un ou l'autre.

**Incident** : aucun nouvel endpoint, réutilise la navigation `onNavigate(view, {incidentId})` déjà câblée pour le clic sur une notification (`Topbar.tsx`) — quitte le planning pour l'écran Incidents.

**Tâche — carte extensible plutôt qu'une navigation (session du 2026-09-21, remplace la version précédente de la même session).** Deux retours directs successifs : (1) le lien ouvrait bien l'écran Tâches mais n'affichait jamais la ligne ciblée — corrigé une première fois (`focusTaskId` force désormais tous les statuts au lieu du filtre par défaut, le scroll attend que la ligne existe réellement dans le DOM au lieu d'un délai fixe, même correctif appliqué à `focusIncidentId`/`IncidentsPage`) ; (2) puis demande explicite de ne pas naviguer du tout — "étendre la card avec les infos de la tâche", mêmes informations et actions (Valider/Rejeter/M'attribuer/Démarrer/Clôturer) que sur la ligne dépliée de l'écran Tâches. Nouveau `TaskCardDialog.tsx` (`features/tasks/`) : charge la tâche complète (`getTask`, `TaskDetail` — le `block.task` nested n'a que 6 champs, insuffisant pour `TaskAccordion`) et les membres du projet (`getProject`, pour le sélecteur de réattribution) au montage, puis monte **directement le composant `TaskAccordion` existant** (celui de `TasksListPage`, aucune duplication de logique) dans une carte centrée plutôt que sous une ligne de tableau — réutilise aussi tel quel `useTaskTransitions`/`CompleteDialog`/`RejectDialog`. `PlanningPage` ouvre cette carte (`dialog.kind === "task"`) au lieu d'appeler `onNavigate` ; `onChanged` (déclenché après toute transition réussie) fait rafraîchir le planning (créneaux, tâches à planifier) sans que la carte ait besoin de connaître ces listes.

### Planning — panneau "À planifier" : tâches et incidents, en accordéon (session du 2026-09-21)

**Statut : implémenté.** Retour direct : le panneau ne listait que les tâches assignées — "j'aimerais qu'il y ait tâches ET incidents", présentés dans un accordéon (deux sections dépliables) plutôt qu'une liste brute, chacun restant glissable sur le calendrier.

- **`IncidentFilterSet` gagne `assigned_to`** (`apps/incidents/filters.py`, simple ajout à `Meta.fields` — django-filter génère l'égalité automatiquement pour un FK). Le champ `assigned_to`/`assigned_to_name` était en réalité **déjà exposé** par `IncidentSerializer` depuis la session du 2026-08-11 (bis) ; seul le paramètre de requête manquait, d'où la limite v1 notée plus haut dans ce fichier. `GET /api/v1/incidents/?assigned_to=<id>&status=signale,en_cours`.
- **Frontend** : nouveau composant générique `components/Accordion.tsx` (+ `Accordion`/`AccordionItem`/`AccordionTrigger`/`AccordionContent`) — idée reprise d'un snippet Base UI fourni, réécrite aux conventions du repo (pas de Tailwind, pas de `@/components/base-ui/*`, pas de Radix), anim `motion` (déjà une dépendance, même patron que `TaskAccordion`). `PlanningPage` : `UnscheduledTask` généralisé en `UnscheduledItem` (union `{kind:"task"}` / `{kind:"incident"}`), deux `AccordionItem` ("Tâches"/"Incidents", ouverts par défaut, avec un compteur), le glisser-déposer distingue les deux via `data.kind` sur l'événement dnd-kit (`createBlock({task: …})` vs `createBlock({incident: …})` — le backend et `ScheduledBlock` supportaient déjà les deux, seul le panneau ne proposait que les tâches).
- Incidents « à planifier » = assignés à l'utilisateur courant et encore ouverts (`signale`/`en_cours`) — même logique que les tâches (`assignee`/`en_cours`), pas de statut "résolu"/"archivé" dans la liste.
- **Tests** : `apps/incidents/tests/test_api.py::IncidentLifecycleApiTests::test_list_filters_by_assigned_to`.

## Synchronisation Outlook (session du 2026-09-22)

**Statut : câblage réel en place, temps réel** (mis à jour le même jour — le tenant Azure AD s'est révélé disponible immédiatement, plus besoin d'attendre un déploiement pour ce point précis). Sens de synchronisation tranché avec l'utilisateur : **Awtodo → Outlook uniquement** (création/modification/suppression), jamais l'inverse — pas de synchro bidirectionnelle, donc pas de résolution de conflit à modéliser. "Temps réel" = déclenché immédiatement à chaque création/modification/suppression, jamais un export manuel ni un batch.

- **`CalendarEvent.outlook_event_id`** (`apps/planning/models.py`, `CharField` blank) : id de l'événement côté Graph une fois créé là-bas.
- **`User.outlook_calendar_sync_enabled`** (`apps/accounts/models.py`, booléen, défaut `False`) : opt-in personnel — champ scalaire simple, reste dans `apps.accounts` comme `planning_color`. Exposé via `MeSerializer`, modifiable via `PATCH /api/v1/accounts/me/planning-preferences/` (`outlook_calendar_sync_enabled`, même endpoint que `planning_color`, mise à jour partielle indépendante).
- **Trois signaux** (`apps/planning/signals.py`) : `calendar_event_created`/`calendar_event_updated`/`calendar_event_cancelled`, émis par `create_event`/`update_event`/`cancel_event` (`apps/planning/services.py`). Un récepteur **par signal** (connaît l'action Graph à effectuer — POST/PATCH/DELETE) planifie une tâche Celery via `transaction.on_commit(...)` — jamais d'appel synchrone dans le flux de la requête HTTP (règle d'architecture #2 du `CLAUDE.md`).
- **`apps/planning/tasks.py::sync_calendar_event_to_outlook`** (tâche `@shared_task`) : vérifie l'opt-in (`owner.outlook_calendar_sync_enabled`), l'`O365Connection` de l'organisation (`is_enabled` + `tenant_id`/`client_id`/`client_secret` renseignés — **réutilise le modèle et l'écran déjà existants pour `Mail.Send`**, un seul jeu d'identifiants Graph par organisation, voir "Module Communication" ci-dessus), puis appelle Microsoft Graph (`apps/planning/graph_client.py`, authentification `msal` en client-credentials, permission d'**application** `Calendars.ReadWrite` — pas de consentement par utilisateur). Cible `POST/PATCH/DELETE /users/{owner.email}/events` — **l'email Awtodo de l'utilisateur doit correspondre à son UPN Microsoft** (confirmé par l'utilisateur pour ce tenant ; pas de nouveau champ de mapping ajouté, `User.azure_oid` — réservé au futur SSO — reste inutilisé ici). N'échoue jamais bruyamment : toute erreur (`GraphSyncError`) est journalisée puis avalée, un incident de synchro Outlook ne doit jamais faire échouer l'action Awtodo qui l'a déclenchée.
- **Backfill à l'activation.** Remonté directement : "ça synchronise pas mes événements déjà existants ?" — en effet, les signaux `calendar_event_*` ne se redéclenchent jamais pour l'historique, donc sans backfill seuls les événements créés *après* avoir coché la synchro apparaîtraient côté Outlook. Nouveau signal `apps.accounts.signals.outlook_calendar_sync_enabled_activated`, émis par `update_planning_preferences` **seulement** sur une bascule `False → True` (pas à chaque enregistrement, pas sur `True → True` ni `True → False`) — respecte la règle de dépendances (`accounts` est plus bas que `planning`, c'est `apps.planning.signals` qui écoute, jamais l'inverse). Le récepteur planifie `apps/planning/tasks.py::backfill_user_outlook_sync(user_id)` (après commit, même pattern que les 3 tâches de synchro) : parcourt les `CalendarEvent` actifs (`status="confirme"`) de l'utilisateur sans `outlook_event_id` et appelle `sync_calendar_event_to_outlook(event_id, "created")` pour chacun — y compris les événements récurrents, le tri traduisible/non traduisible étant géré par la tâche unitaire elle-même (voir ci-dessus). **Rattrapage ponctuel uniquement** : un utilisateur qui avait déjà activé la synchro avant ce correctif doit décocher/recocher la case pour déclencher le backfill sur ses événements déjà existants.
- **Événements récurrents — traduits vers Graph (revu le même jour, un vrai tenant étant devenu disponible pour tester).** Graph n'accepte pas une chaîne RRULE : il attend un objet structuré `{pattern, range}`. `apps/planning/graph_client.py::build_graph_recurrence(event)` traduit le sous-ensemble RRULE que l'éditeur front peut réellement produire (`frontend/src/features/planning/recurrence.ts` — `FREQ` daily/weekly/monthly/yearly, `INTERVAL`, `BYDAY` sur weekly seulement, fin par `UNTIL` ou `COUNT`) :
  - `FREQ=DAILY` → `pattern.type="daily"`
  - `FREQ=WEEKLY` → `pattern.type="weekly"`, `daysOfWeek` = `BYDAY` traduit (`MO`→`monday`…) ou, si absent, le jour de semaine de `event.start`
  - `FREQ=MONTHLY` → `pattern.type="absoluteMonthly"`, `dayOfMonth` = jour de `event.start`
  - `FREQ=YEARLY` → `pattern.type="absoluteYearly"`, `dayOfMonth`/`month` = ceux de `event.start`
  - `UNTIL` → `range.type="endDate"` ; `COUNT` → `range.type="numbered"` ; ni l'un ni l'autre → `range.type="noEnd"`

  **Le backend accepte une RRULE plus riche que ce que l'éditeur produit** (déjà noté ailleurs, `_validate_recurrence_rule` ne restreint pas au sous-ensemble front) — une règle passée directement par l'API avec une construction non couverte (ex. `BYMONTHDAY`, fréquence infra-journalière déjà refusée à la création) fait renvoyer `None` par `build_graph_recurrence` : la tâche l'ignore et journalise plutôt que d'envoyer un motif approximatif à Outlook, même comportement défensif qu'avant, juste un périmètre plus large de cas réellement synchronisés. **Toute la série se synchronise comme un seul événement Graph**, jamais occurrence par occurrence — cohérent avec la règle déjà actée ("l'édition ne porte jamais sur une occurrence isolée", pas d'EXDATE/RECURRENCE-ID). Testé unitairement (`apps/planning/tests/test_graph_recurrence.py`, la fonction pure sans DB) ; **pas encore vérifié contre une vraie boîte Outlook** — à confirmer par l'utilisateur au premier événement récurrent réellement synchronisé.
- **Celery — pas de broker Redis provisionné.** Nouveau `config/celery.py` (app Celery standard, autodiscover) + réglages dans `config/settings/base.py` : `CELERY_TASK_ALWAYS_EAGER=True` par défaut (env `CELERY_TASK_ALWAYS_EAGER`) — la tâche s'exécute immédiatement dans le même processus, sans broker, mais reste écrite comme une vraie tâche Celery asynchrone (`.delay(...)`), prête à basculer en réellement async dès qu'un Redis sera provisionné (juste repasser le réglage à `False` + renseigner `CELERY_BROKER_URL` + lancer `celery -A config worker`, aucun changement de code métier). Nouvelles dépendances `msal`/`requests` (`requirements/base.txt`).
...
- **Créneaux de tâche/incident synchronisés (session du 2026-09-23) — un seul destinataire, pas de partage.** Remonté directement : "il faut aussi synchroniser les tâches planifiées (titre = nom de la tâche)". `ScheduledBlock.outlook_event_id` (scalaire, comme `CalendarEvent.outlook_event_id`) — un créneau n'a qu'un seul destinataire possible, son `owner` : une tâche/un incident n'a qu'un assigné, aucune notion de partage (`Task.assignee` est un FK unique, pas de `EventParticipant` équivalent pour `ScheduledBlock`). **Point explicitement corrigé après une première implémentation erronée** : une première passe avait fait suivre les créneaux vers chaque bénéficiaire d'un `CalendarShare` (partage de calendrier) du propriétaire — l'utilisateur a corrigé directement : *"si j'ai un calendrier partagé : PAS DE SYNCHRO OUTLOOK DE TOUT MON CALENDRIER !!!"*. Le partage de calendrier (`CalendarShare`) reste et restera un pur overlay de lecture **dans Awtodo uniquement**, sans aucun effet sur Outlook, pour quiconque.
  - **Trois signaux** `scheduled_block_created`/`updated`/`cancelled` (`apps/planning/signals.py`), émis par `create_block`/`update_block`/`cancel_block`, un récepteur par signal qui planifie `apps/planning/tasks.py::sync_scheduled_block_to_outlook(block_id, action)` après commit — structure identique à `sync_calendar_event_to_outlook` (même opt-in requis sur le propriétaire, mêmes garde-fous, même gestion d'erreurs avalées).
  - **Titre Outlook = titre de la tâche ou de l'incident planifié** (`apps/planning/graph_client.py::_block_payload`, pas de description/lieu/récurrence — le modèle `ScheduledBlock` n'en a pas).
  - **Backfill** couvre les créneaux du propriétaire sans `outlook_event_id` (`backfill_user_outlook_sync`), même logique que les événements personnels.

- **Événement partagé avec un ou plusieurs participants — synchronisé sur l'Outlook de chacun (session du 2026-09-23).** Cadré précisément avec l'utilisateur, deux mécanismes de partage bien distincts dans Awtodo et deux comportements Outlook différents : partager **tout son calendrier** (`CalendarShare`) ne synchronise **jamais rien** ; partager **un événement précis** (`EventParticipant`, inviter quelqu'un à un `CalendarEvent`) synchronise cet événement sur l'Outlook du participant — s'il a lui-même activé son opt-in (l'invitation ne contourne pas ce contrôle individuel, cohérent avec le reste de la synchro).
  - **Participants dès la création, pas seulement après coup** (retour direct : "possible de faire en sorte qu'on puisse ajouter un participant dès la création d'un évènement ?"). `create_event` accepte un `participant_ids` optionnel (`EventCreateSerializer.participant_ids`, liste d'ids utilisateur) — après création de l'événement, appelle `add_participant` un par un pour chacun. Aucune logique de synchro dupliquée : chaque ajout déclenche exactement le même signal `event_participant_invited` qu'un ajout après coup, donc la même tâche `sync_event_participant_to_outlook`. L'organisateur présent dans `participant_ids` (ex. sélection malencontreuse côté UI) est silencieusement ignoré plutôt que de faire échouer toute la création. Frontend : `EventDialog.tsx` gagne un bloc "Participants" identique à celui du mode édition, mais opérant sur une liste locale (`pendingParticipantIds`) tant que l'événement n'existe pas encore — envoyée dans le payload de création uniquement (`EventUpdateSerializer` ne connaît pas ce champ, aucun risque de l'envoyer par erreur sur une modification).
  - **Nouveau modèle `EventParticipantOutlookSync`** (`event`, `user`, `outlook_event_id`, `UniqueConstraint(event, user)`) : un id Graph **par participant**, distinct de `CalendarEvent.outlook_event_id` qui ne couvre que la copie de l'organisateur.
  - **Nouveau signal `event_participant_removed`** (`apps/planning/signals.py`), émis par `remove_participant` (n'émettait encore aucun signal) — symétrique à `event_participant_invited` (déjà existant, consommé par `apps.notifications`).
  - **Quatre tâches** (`apps/planning/tasks.py`) : `sync_event_participant_to_outlook(event_id, user_id, action)` synchronise **la copie d'un seul participant** — déclenchée par `event_participant_invited` (`action="created"`) et `event_participant_removed` (`action="cancelled"`, supprime uniquement *sa* copie, pas l'événement des autres) ; `sync_event_to_all_participants_outlook(event_id, action)` fait suivre un changement de **contenu** de l'organisateur (renommage, déplacement, annulation) vers la copie de **chaque participant actif** — déclenchée par `calendar_event_updated`/`calendar_event_cancelled` (et `calendar_event_created`, sans effet à la création puisqu'il ne peut pas encore y avoir de participant). Chaque participant est traité indépendamment : l'échec Graph pour l'un n'empêche pas la synchro des autres.
  - **Backfill étendu** : `backfill_user_outlook_sync` couvre aussi les événements auxquels l'utilisateur qui active son opt-in est déjà invité en tant que participant (`EventParticipant.objects.filter(user_id=...)`), et fait suivre le fan-out participants pour les événements dont il est lui-même l'organisateur.
  - **Limite assumée, documentée dans le code** : même limite que pour les créneaux avant leur simplification — pas de rattrapage rétroactif si un partage d'événement change alors que le participant n'a pas encore activé son opt-in au moment de l'invitation ; c'est justement à ça que sert le backfill à l'activation.

- **Occurrence unique d'une série modifiée/supprimée sans toucher au reste (session du 2026-09-23) — RECURRENCE-ID.** Retour direct, inversant une décision actée à la conception du module Planning ("l'édition ne porte jamais sur une occurrence isolée") : *"actuellement, si on modifie une des récurrences, ça modifie TOUTE la série. il ne faut pas ça. [...] si on modifie un évènement de la série, ça ne doit modifier que l'évènement (pareil pour la suppression). ça suit avec la synchro O365 évidemment."*
  - **Nouveau modèle `CalendarEventOccurrenceOverride`** (`event`, `original_start`, `is_cancelled`, plus une copie complète des champs éditables — `title`/`description`/`location`/`start`/`end`/`all_day`). `original_start` est la clé stable qui identifie "quelle occurrence" (l'heure qu'elle aurait eue sans override, calculée depuis la RRULE d'origine — équivalent du RECURRENCE-ID iCal) ; ne bouge jamais même si `start` est déplacé par l'override. **Simplification assumée** : une occurrence overridée devient entièrement indépendante du modèle (copie complète à la création de la ligne, pas un diff partiel par champ) — un futur renommage de la série n'affecte plus une occurrence déjà overridée. Plus simple à raisonner et à synchroniser qu'un héritage de champs partiel ; assumé comme un compromis correct pour ce périmètre. Jamais supprimée physiquement — `is_cancelled` plutôt qu'un retrait de la ligne.
  - **Nouvelles fonctions de service** `update_event_occurrence`/`cancel_event_occurrence` (`apps/planning/services.py`) : valident que `occurrence_start` correspond bien à une occurrence réelle de la RRULE (`rrulestr(...).between(occ, occ, inc=True)`) avant de créer/retrouver la ligne d'override. `update_event`/`cancel_event` existants restent la façon d'agir sur **toute** la série, inchangés.
  - **`expand_occurrences` (fonction pure, inchangée) + nouveau post-traitement `_merge_calendar_event_occurrence_overrides`** : appliqué aux deux points de `get_calendar` qui listent des `CalendarEvent` (les miens, et ceux d'un calendrier partagé) — une occurrence annulée disparaît de la liste, une occurrence modifiée prend les valeurs de son override. **Pas appliqué à `ProjectPlanningEntry`** (planning de projet), hors périmètre de cette demande. `_event_occurrence_dict` gagne `is_overridden` (distinct de `is_recurring`, qui reste vrai pour toute la série).
  - **API** : `POST /planning/events/{id}/occurrences/update/` (body : `occurrence_start` + champs à changer) et `POST /planning/events/{id}/occurrences/cancel/` (body : `occurrence_start`) — `occurrence_start` en body plutôt qu'en URL pour éviter l'encodage d'un datetime dans un chemin.
  - **Synchronisation Outlook — "ça suit avec la synchro O365 évidemment".** Graph modélise nativement une occurrence isolée : chaque instance d'une série a son propre id, distinct de celui du modèle, résolu via `GET /events/{master}/instances` (nouveau `apps/planning/graph_client.py::_resolve_instance_id`, comparaison par instance la plus proche de `occurrence_start` plutôt qu'une correspondance exacte de chaîne — Graph renvoie ses horodatages dans le fuseau UTC par défaut, une correspondance exacte serait fragile). PATCHer/DELETEer cet id d'instance (`upsert_graph_occurrence`/`delete_graph_occurrence`) plutôt que celui du modèle ne touche que cette occurrence. **Nouveau modèle `EventOccurrenceOutlookSync`** (`override`, `user`, `outlook_event_id`) : un id Graph par (occurrence, destinataire) — chaque destinataire (organisateur + participants) a sa propre copie de la série, donc sa propre instance Graph pour "la même" occurrence logique. Trois nouvelles tâches (`sync_event_occurrence_to_outlook` pour l'organisateur, `sync_event_occurrence_for_participant_outlook` pour un participant précis, `sync_event_occurrence_to_all_participants_outlook` en fan-out) — ne font rien si la série elle-même (ou la copie du destinataire visé) n'est pas encore synchronisée, rien à cibler. **⚠️ Best-effort, non vérifié contre un vrai tenant Outlook** — même disclaimer que la traduction de récurrence : à confirmer par l'utilisateur au premier usage réel, la logique de résolution d'instance pourrait nécessiter un ajustement.
  - **Frontend** : nouveau `EventOccurrenceDialog.tsx` (formulaire simplifié — pas de récurrence, pas de participants, ces deux-là restant au niveau de la série). Cliquer une occurrence d'une série récurrente (`WeekGrid`/`MonthGrid` → `PlanningPage.handleItemClick`) affiche d'abord un choix de portée ("Cette occurrence uniquement" / "Toute la série") avant d'ouvrir le bon dialogue — un événement ponctuel garde le comportement direct existant.

- **Tests** : `apps/planning/tests/test_outlook_sync_signals.py` (signaux d'événements libres, de créneaux, de participants et d'occurrences uniques, récepteurs qui planifient la tâche sans jamais lever, backfill planifié une seule fois à l'activation) ; `apps/planning/tests/test_outlook_sync_tasks.py` (tâche d'événement libre côté organisateur) ; `apps/planning/tests/test_outlook_sync_block_tasks.py` (tâche de créneau, propriétaire seul) ; `apps/planning/tests/test_outlook_sync_participant_tasks.py` (copie participant : opt-in propre au participant indépendant de celui de l'organisateur, mise à jour, retrait, isolation des erreurs par participant dans le fan-out, backfill des événements où l'utilisateur est invité) ; `apps/planning/tests/test_outlook_sync_occurrence_tasks.py` (nouveau — copie d'occurrence : série pas encore synchronisée = rien à cibler, mise à jour une fois synchronisée, annulation, erreur Graph avalée, copie participant utilisant sa propre série, fan-out) ; `apps/planning/tests/test_graph_recurrence.py` (traduction RRULE → Graph, fonction pure) ; `apps/planning/tests/test_api.py` (occurrence unique via l'API — modification n'affecte pas la série, annulation ne retire que cette occurrence, rejet sur événement non récurrent ou date hors série) ; `apps/accounts/tests/test_account_access.py::PlanningPreferenceServiceTests` (cas pour `outlook_calendar_sync_enabled` + émission du signal seulement sur `False → True`).
...
`location` omis du corps si `event.location` est vide (pas de clé envoyée plutôt qu'une chaîne vide). `recurrence` ajouté si `event.recurrence_rule` est traduisible (voir ci-dessus, `build_graph_recurrence`) — exemple pour `FREQ=WEEKLY;BYDAY=MO,WE` démarrant un vendredi :

```json
"recurrence": {
  "pattern": { "type": "weekly", "interval": 1, "daysOfWeek": ["monday", "wednesday"] },
  "range": { "type": "noEnd", "startDate": "2026-10-02" }
}
```
...
**Créneaux de tâche/incident (`ScheduledBlock`, session du 2026-09-23)** — mêmes fonctions HTTP génériques (`_post_event`/`_patch_event`/`delete_graph_event`, extraites pour être partagées), payload différent (`_block_payload`, pas de `body`/`location`/`recurrence`, le modèle n'en a pas). **Un seul destinataire possible**, `block.owner.email` — pas de partage pour les créneaux (voir plus haut) :

| Signal / action | Requête |
|---|---|
| `scheduled_block_created` (pas d'id Graph) | `POST /users/{owner.email}/events` |
| `scheduled_block_updated` (id Graph déjà présent) | `PATCH /users/{owner.email}/events/{outlook_event_id}` |
| `scheduled_block_cancelled` | `DELETE /users/{owner.email}/events/{outlook_event_id}`, seulement si un id existait |

```json
{
  "subject": "<block.task.title ou block.incident.title>",
  "start": { "dateTime": "<block.start.isoformat()>", "timeZone": "Europe/Paris" },
  "end": { "dateTime": "<block.end.isoformat()>", "timeZone": "Europe/Paris" }
}
```

**Copie d'un participant invité (`EventParticipant`, session du 2026-09-23)** — réutilise `_event_payload`/`create_graph_event`/`update_graph_event`/`delete_graph_event` tels quels (même contenu que la copie de l'organisateur), seul `{upn}` change :

| Signal / action | Requête | `{upn}` |
|---|---|---|
| `event_participant_invited` (pas d'id Graph pour ce participant) | `POST /users/{upn}/events` | email du participant |
| `calendar_event_updated`/`calendar_event_created` fan-out (id déjà présent) | `PATCH /users/{upn}/events/{outlook_event_id}` | email de chaque participant actif |
| `event_participant_removed` ou `calendar_event_cancelled` fan-out | `DELETE /users/{upn}/events/{outlook_event_id}` | idem, seulement si un id existait |

**Occurrence unique d'une série (`CalendarEventOccurrenceOverride`, session du 2026-09-23)** — ⚠️ best-effort, non vérifié contre un vrai tenant. `{master}` = `outlook_event_id` du modèle (organisateur) ou de la copie du destinataire (participant) :

| Étape | Requête |
|---|---|
| Résolution de l'instance (avant tout PATCH/DELETE) | `GET /users/{upn}/events/{master}/instances?startDateTime=...&endDateTime=...` (fenêtre ±2h autour de `occurrence_start`, en UTC ; instance la plus proche retenue, pas de correspondance exacte de chaîne) |
| `calendar_event_occurrence_updated` (instance résolue) | `PATCH /users/{upn}/events/{instance_id}` avec `{subject, body, start, end, isAllDay, location?}` tirés de l'override — pas de `recurrence` (ça ne concerne que le modèle) |
| `calendar_event_occurrence_cancelled` | `DELETE /users/{upn}/events/{instance_id}` |
