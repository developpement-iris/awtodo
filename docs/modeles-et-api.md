...
- **Créneaux de tâche/incident synchronisés (session du 2026-09-23), y compris vers les calendriers partagés.** Remonté directement : "il faut aussi synchroniser les tâches planifiées (titre = nom de la tâche), et si une tâche est partagée, la synchroniser sur le calendrier de toutes les personnes à qui elle est partagée." Deux points cadrés avec l'utilisateur avant d'implémenter : (1) "partagée" = le partage de calendrier déjà existant (`CalendarShare`), pas une notion de tâche multi-assignée (qui n'existe pas — `Task.assignee` est un FK unique) ; (2) **le partage seul ne suffit pas** — un destinataire ne reçoit le créneau sur son Outlook que s'il a lui-même activé son opt-in ("quand on partage un calendrier ça ne le synchronise pas sur l'outlook de l'autre personne"), cohérent avec le principe déjà en place que chacun contrôle ce qui atterrit dans son propre Outlook. S'applique aux tâches **et** aux incidents (même modèle technique `ScheduledBlock`).
  - **Nouveau modèle `BlockOutlookSync`** (`block`, `user`, `outlook_event_id`, `UniqueConstraint(block, user)`) : un id Graph **par destinataire**, contrairement à `CalendarEvent.outlook_event_id` (scalaire, un seul destinataire possible pour un événement personnel). Jamais supprimé physiquement — `outlook_event_id` vidé à l'annulation, comme pour `CalendarEvent`.
  - **Trois signaux** `scheduled_block_created`/`updated`/`cancelled` (`apps/planning/signals.py`), émis par `create_block`/`update_block`/`cancel_block` (`apps/planning/services.py`), un récepteur par signal qui planifie `apps/planning/tasks.py::sync_scheduled_block_to_outlook(block_id, action)` après commit — même schéma que les événements libres.
  - **La tâche calcule les destinataires à l'exécution** : le propriétaire du créneau + tous les `grantee` d'un `CalendarShare` actif où il est `owner`, filtrés ensuite un par un sur `outlook_calendar_sync_enabled` (chacun sa propre `O365Connection` via `recipient.organisation` — généralement la même organisation, mais pas de contrainte cross-org sur `CalendarShare`). Chaque destinataire est traité indépendamment (`try`/`except` par destinataire) : l'échec Graph pour l'un n'empêche pas la synchro des autres.
  - **Titre Outlook = titre de la tâche ou de l'incident planifié** (`apps/planning/graph_client.py::_block_payload`, pas de description/lieu — le modèle `ScheduledBlock` n'en a pas). Pas de récurrence non plus (`ScheduledBlock` n'en supporte pas).
  - **Backfill étendu** : `backfill_user_outlook_sync` couvre maintenant aussi les créneaux — les siens, et ceux des personnes qui ont partagé leur calendrier avec l'utilisateur qui vient d'activer son opt-in (cas symétrique : un partage déjà en place avant l'activation du destinataire doit aussi se rattraper).
  - **Limite assumée, documentée dans le code** : la liste des destinataires est recalculée à chaque exécution à partir des partages *actuellement* actifs — révoquer un partage après coup ne supprime pas rétroactivement l'événement déjà créé sur l'Outlook de l'ex-destinataire.
- **Tests** : `apps/planning/tests/test_outlook_sync_signals.py` (signaux d'événements libres et de créneaux, récepteurs qui planifient la tâche sans jamais lever, backfill planifié une seule fois à l'activation) ; `apps/planning/tests/test_outlook_sync_tasks.py` (tâche d'événement libre, appels Graph mockés) ; `apps/planning/tests/test_outlook_sync_block_tasks.py` (nouveau — tâche de créneau : propriétaire seul, partage avec/sans opt-in, partage révoqué, mise à jour, annulation, isolation des erreurs par destinataire, backfill des créneaux partagés) ; `apps/planning/tests/test_graph_recurrence.py` (traduction RRULE → Graph, fonction pure) ; `apps/accounts/tests/test_account_access.py::PlanningPreferenceServiceTests` (cas pour `outlook_calendar_sync_enabled` + émission du signal seulement sur `False → True`).
...
**Créneaux de tâche/incident (`ScheduledBlock`, session du 2026-09-23)** — mêmes fonctions HTTP génériques (`_post_event`/`_patch_event`/`delete_graph_event`, extraites pour être partagées), payload différent (`_block_payload`, pas de `body`/`location`/`recurrence`, le modèle n'en a pas) :

| Signal / action | Requête | `{upn}` |
|---|---|---|
| `scheduled_block_created` (pas d'id Graph pour ce destinataire) | `POST /users/{upn}/events` | pour chaque destinataire (propriétaire + partages actifs avec opt-in) |
| `scheduled_block_updated` (id Graph déjà présent pour ce destinataire) | `PATCH /users/{upn}/events/{outlook_event_id}` | idem |
| `scheduled_block_cancelled` | `DELETE /users/{upn}/events/{outlook_event_id}` | idem, seulement si un id existait |
