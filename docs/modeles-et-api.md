...
**⚠️ Bloc (1) déplacé — session du 2026-09-22 (suite Outlook), voir plus bas** : le formulaire d'édition de `O365Connection` n'a plus sa place sur un onglet **par projet** alors que la connexion est **par organisation** — remonté directement par l'utilisateur ("on fout pas ça dans Communication"). Déplacé vers `Administration > Intégrations` (nouvel écran, admin d'organisation uniquement). `CommunicationTab.tsx` ne garde qu'un statut en lecture seule (configurée/non configurée), plus de formulaire.
...
- **Configuration de `O365Connection` déplacée vers l'écran Administration.** Retour direct de l'utilisateur : le formulaire tenant/client/secret n'avait pas sa place dans l'onglet Communication d'un **projet** alors que la connexion est **par organisation**. Nouvelle sous-section « Intégrations » sur l'écran Administration (`frontend/src/features/administration/IntegrationsSection.tsx`, 5ᵉ onglet de `AdministrationPage.tsx` — visible si `is_platform_admin` ou `organisation_role === "admin"`, même droit que le backend `is_organisation_admin`) : seul endroit désormais où le formulaire d'édition existe. `CommunicationTab.tsx` (onglet Communication d'un projet) ne garde plus qu'un statut en lecture seule (configurée/non configurée) pointant vers Administration — aucun changement d'API, `O365Connection` reste organisation-scoped comme avant.
