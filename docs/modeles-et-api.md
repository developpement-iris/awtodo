...
- **Backfill à l'activation.** Remonté directement : "ça synchronise pas mes événements déjà existants ?" — en effet, les signaux `calendar_event_*` ne se redéclenchent jamais pour l'historique, donc sans backfill seuls les événements créés *après* avoir coché la synchro apparaîtraient côté Outlook. Nouveau signal `apps.accounts.signals.outlook_calendar_sync_enabled_activated`, émis par `update_planning_preferences` **seulement** sur une bascule `False → True` (pas à chaque enregistrement, pas sur `True → True` ni `True → False`) — respecte la règle de dépendances (`accounts` est plus bas que `planning`, c'est `apps.planning.signals` qui écoute, jamais l'inverse). Le récepteur planifie `apps/planning/tasks.py::backfill_user_outlook_sync(user_id)` (après commit, même pattern que les 3 tâches de synchro) : parcourt les `CalendarEvent` actifs (`status="confirme"`) de l'utilisateur, non récurrents, sans `outlook_event_id`, et appelle `sync_calendar_event_to_outlook(event_id, "created")` pour chacun. **Rattrapage ponctuel uniquement** : un utilisateur qui avait déjà activé la synchro avant ce correctif doit décocher/recocher la case pour déclencher le backfill sur ses événements déjà existants.
- **Limite v1 assumée : les événements récurrents ne sont pas synchronisés.** Traduire une RRULE iCal vers le format de récurrence Graph n'a pas pu être vérifié faute d'accès à une vraie boîte Outlook pour tester bout-en-bout — plutôt que risquer une série mal traduite côté Outlook, la tâche détecte `event.recurrence_rule` non vide et ignore (avec un log), sans lever d'erreur. À lever plus tard, une fois testable en conditions réelles.
