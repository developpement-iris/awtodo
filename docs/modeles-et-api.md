...
**Correctif (session du 2026-09-21)** : le lien ouvrait bien l'écran Tâches/Incidents mais n'affichait jamais la ligne ciblée (retour direct : "ouvre la liste mais pas la tâche en particulier"). Deux causes cumulées dans `TasksListPage`/`IncidentsPage` — mêmes correctifs dans les deux : (1) le fetch appliquait le filtre de statut par défaut (masque `archivée`/`rejetée`), qui pouvait exclure la ligne ciblée selon son statut — `focusTaskId`/`focusIncidentId` force désormais la sélection de **tous** les statuts au lieu du filtre par défaut. (2) Le scroll-vers-la-ligne utilisait un délai fixe de 50 ms indépendant du chargement réseau, qui pouvait s'exécuter avant que la ligne existe dans le DOM (`getElementById` échouait silencieusement) — remplacé par un effet déclenché une fois les données chargées et la ligne confirmée présente.
