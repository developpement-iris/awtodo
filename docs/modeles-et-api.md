...
### Planning — personnalisation (couleur, horaires de travail)

**Statut : implémenté (même session).** Deux réglages, demandés ensemble : la couleur de son propre calendrier, et des horaires de travail qui grisent le reste de la grille en vue Semaine — placés dans un panneau dédié sur l'écran Planning lui-même (bouton "Mon planning"), pas dans l'écran Paramètres, choix fait avec l'utilisateur.

- **Modèles** : `User.planning_color` (`CharField(7)`, blank, validé `^#[0-9A-Fa-f]{6}$` ou vide — vide = repli sur l'accent thémé, comportement inchangé pour qui n'a jamais rien réglé). `User.work_hours_start`/`work_hours_end` (`TimeField`, défaut `09:00`/`18:00`).
- **`planning_color` exposé sur `UserSerializer` de base**, pas seulement `MeSerializer` — à la différence d'`email_notifications_enabled`, ce n'est pas une donnée privée : un calendrier partagé pourrait un jour afficher la couleur choisie par son propriétaire plutôt que la rotation automatique de palette (non fait dans cette passe — `CalendarUser`, le sérialiseur allégé utilisé par `apps.planning`, ne l'expose pas encore ; seul l'usage "ma propre vue de mon propre calendrier" est câblé). `work_hours_start`/`work_hours_end` restent sur `MeSerializer` uniquement : ils ne grisent jamais la grille de quelqu'un d'autre.
- **`PATCH /api/v1/accounts/me/planning-preferences/`** — mise à jour partielle, chaque champ indépendant des deux autres. `update_planning_preferences` (`apps/accounts/services.py`) valide `work_hours_start < work_hours_end` (utilisant la valeur déjà en base pour le champ non fourni, si un seul des deux est modifié).
- **Frontend** : `PlanningPreferencesDialog.tsx` (même patron `planning-dialog` que `BlockDialog`/`EventDialog`) — palette de couleurs restreinte à 6 teintes sobres déjà utilisées pour l'identité des calendriers partagés (`PLANNING_PALETTE`, `features/planning/types.ts`, remplace la constante locale `CALENDAR_COLORS` de `PlanningPage.tsx`) plutôt qu'un `<input type="color">` natif — impossible à styler pour respecter la charte, et risque de couleurs criardes hors palette. Deux `<input type="time">` pour les horaires, mêmes styles que les autres champs de formulaire planning (`.planning-field input`). `PlanningPage` charge son propre `Me` via `getMe()` (indépendant du contexte global `currentUser`, qui n'expose pas les horaires) ; `WeekGrid` reçoit `workHours` et grise (`.week-grid__offhours`, non cliquable — visuel seulement, ne bloque ni la création ni le glisser-déposer) les bandes avant/après la plage réglée, dans les bornes de la grille (7h–21h).
- **Tests** : `apps/accounts/tests/test_account_access.py::PlanningPreferenceServiceTests` + tests API dans `SettingsApiTests`.
