...
## Synchronisation Outlook (scaffolding — session du 2026-09-22)

**Statut : scaffolding, comme le module Communication ci-dessus — aucun appel Graph réel n'est émis.** Question posée directement ("facile à faire pour le réexporter une fois sur AWS ?") : la réponse est que la plateforme d'hébergement n'a aucune influence ici — les deux vrais blocages sont un tenant Azure AD réel (même blocage que le SSO) et Celery/Redis (dépendance déclarée, jamais câblée, voir CLAUDE.md > Stack technique). Sens de synchronisation tranché avec l'utilisateur : **Awtodo → Outlook uniquement** (création/modification/suppression), jamais l'inverse — pas de synchro bidirectionnelle, donc pas de résolution de conflit à modéliser.

- **`CalendarEvent.outlook_event_id`** (`apps/planning/models.py`, `CharField` blank) : id de l'événement côté Graph une fois créé là-bas — vide tant que rien n'est câblé.
- **`User.outlook_calendar_sync_enabled`** (`apps/accounts/models.py`, booléen, défaut `False`) : opt-in personnel — champ scalaire simple, reste dans `apps.accounts` comme `planning_color` (pas de modélisation relationnelle nécessaire, à la différence des horaires de travail qui ont déménagé vers `apps.planning`). Exposé via `MeSerializer`, modifiable via `PATCH /api/v1/accounts/me/planning-preferences/` (`outlook_calendar_sync_enabled`, même endpoint que `planning_color`, mise à jour partielle indépendante).
- **Trois nouveaux signaux** (`apps/planning/signals.py`) : `calendar_event_created`/`calendar_event_updated`/`calendar_event_cancelled`, émis par `create_event`/`update_event`/`cancel_event` (`apps/planning/services.py`). Un seul récepteur `sync_event_to_outlook` connecté sur les trois — **volontairement inerte** (`return None`), même principe que `apps.communication.signals.notify_channels_on_incident_created`. La docstring du récepteur documente le câblage prévu au déploiement (vérifier l'opt-in + `O365Connection`, tâche Celery asynchrone, appels Graph `Calendars.ReadWrite` en permission d'**application** — pas de consentement par utilisateur, même modèle que `Mail.Send` pour `O365Connection` existant — traduction de `recurrence_rule` vers le champ `recurrence` de Graph, une série entière à la fois, jamais occurrence par occurrence).
- **Frontend** : nouvelle case "Synchroniser mon calendrier vers Outlook" dans `PlanningPreferencesDialog.tsx` (panneau "Mon planning"), à côté de la couleur de calendrier — texte explicite "Pas encore actif" pour ne pas laisser croire à une fonctionnalité réelle (même règle que le bandeau du module Communication).
- **Tests** : `apps/planning/tests/test_outlook_sync_signals.py` (les 3 signaux sont bien émis, le récepteur inerte ne casse rien) ; `apps/accounts/tests/test_account_access.py::PlanningPreferenceServiceTests` (nouveaux cas pour `outlook_calendar_sync_enabled`).
